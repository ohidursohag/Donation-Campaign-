...
const DonationCard = ({ cardData }) => {
   const {id, Picture, Text_color, Title, Card_bg, Category, Category_bg ,} = cardData || {}
   ...
   return (
      <Link to={`/donation-details/${id}`} state={Title}
         ...
         <div className='p-5 space-y-3'>
            <p style={{ backgroundColor: `${Category_bg}` }} 
               className={`px-3 py-1 w-max rounded-md font-medium`}>{Category}</p>
